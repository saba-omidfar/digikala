...
const getAndShowAllHeaderSliders = async () => {
  const headerSliderContainer = document.querySelector(
    "#header-slider-container"
  );

  const res = await fetch(`https://digikala-backend.vercel.app/header-slider`);
  const headerSlides = await res.json();

  headerSliderContainer.innerHTML = "";

  headerSlides.forEach((slide) => {
    headerSliderContainer.insertAdjacentHTML(
      "beforeend",
      `
...
const showMoreServices = async () => {
  const showMoreServicesModal = document.querySelector(
    ".show-services__modal-bg"
  );
  const closeServiceModalIcon = document.querySelector(
    ".show-service__modal-icon"
  );
  const showMoreServicesWrapper = document.querySelector(
    "#show-more-services-wrapper"
  );
  const digikalaGroupServicesWrapper = document.querySelector(
    "#digikala-group-services-wrapper"
  );

  const res = await fetch(`http://localhost:5000/services`);
  const services = await res.json();

  let shuffledServices = [...services].sort((a, b) => 0.5 - Math.random());
  showMoreServicesWrapper.innerHTML = "";

  // Insert Shuffled Services To Header
  shuffledServices.splice(0, 10).forEach((service) => {
    showMoreServicesWrapper.insertAdjacentHTML(
      "beforeend",
      `
            <div class="col-2 p-2">
  ...
  services.forEach((service) => {
    if (service.desc) {
      digikalaGroupServicesWrapper.insertAdjacentHTML(
        "beforeend",
        `
...
const getAndShowAllIncredibleOffers = async () => {
  const incredibleOffersContainer = document.querySelector(
    "#icredible-offers-container"
  );
  const incredibleProductsContainer = document.querySelector(
    ".incredible-products"
  );
  const showMoreIncredibleOffersIcon = document.querySelector(
    ".incredible-products__btn-text"
  );
  ...
  incredibleOffersContainer.insertAdjacentHTML(
    "beforeend",
    `
  ...
  const showAllIncredibleOffers = document.querySelector(
    ".slide-show-all__wrapp"
  );
  let discountPrice;

  incredibleOffers.forEach((incredibleOffer) => {
    showAllIncredibleOffers.insertAdjacentHTML(
      "beforebegin",
      `
            <div class="swiper-slide">
                <a href="product.html?name=${
                  incredibleOffer.shortName
                }&page=1" class="border-right bg-white d-block position-relative flex-grow-1 py-3 px-4 px-lg-3 px-xs-3"  style="height: 225px;">
                                ...
                                <div class="pt-1 d-flex flex-column align-items-stretch justify-content-between">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div class="item-price-percent__wrapp px-1 d-flex justify-content-center align-items-center">
                                            <span class="item-price-percent">${new Intl.NumberFormat(
                                              "fa-IR"
                                            ).format(
                                              incredibleOffer.discount
                                            )}%</span>
                                        </div>
                                        <div class="d-flex align-items-center justify-content-end">
                                        <span class="item-price">${new Intl.NumberFormat(
                                          "fa-IR"
                                        ).format(incredibleOffer.price)}</span>
  ...
  incredibleOffers.forEach((incredibleOffer) => {
    incredibleProductsContainer.insertAdjacentHTML(
      "beforeend",
      `
            <div class="incredible-product-item d-flex justify-content-center align-items-center p-2 ms-2">
                <div class="incredible-product-item-radius">
                    <img class="incredible-product-item-img" src="assets/covers/${
                      incredibleOffer.images[0].url
                    }"
                </div>
                <div class="incredible-product-item__price-wrapp px-1 d-flex align-items-center justify-content-center position-absolute">
                    <span class="incredible-product-item__price">${new Intl.NumberFormat(
                      "fa-IR"
                    ).format(incredibleOffer.discount)}%</span>
                </div>
            </div>
        `
    );
  ...
  showMoreIncredibleOffersIcon.addEventListener(
    "click",
    goToIncredibleOffersPage
  );
};
...
const getAndShowAllAdBanners = async () => {
  ...
  const digikalaBannersContainer = document.querySelector(
    "#digikala-banners-container"
  );
  ...
  (adBannersContainer.innerHTML = ""),
    ...
    [...ads].splice(0, 4).forEach((ad) => {
      adBannersContainer.insertAdjacentHTML(
        "beforeend",
        `
    ...
    [...ads].splice(4, 2).forEach((ad) => {
      bannersWrapper.insertAdjacentHTML(
        "beforeend",
        `
    ...
    [...ads].splice(6, 2).forEach((ad) => {
      digikalaBannersContainer.insertAdjacentHTML(
        "beforeend",
        `
...
const getAndShowAllPopularBrands = async () => {
  const popularBrandsContainer = document.querySelector(
  ...
  popularBrands.forEach((brand) => {
    popularBrandsContainer.insertAdjacentHTML(
      "beforeend",
      `
            <div class="swiper-slide">
...
const addProductToUserBasket = async (product) => {
  if (isUserLogin) {
    ...
    showSwal("لطفا ابتدا وارد شوید", "error", "ok", () => {
...
const deleteProductFromBasket = async (productID) => {
  ...
  if (userID) {
    const res = await fetch(
      `http://localhost:5000/basket/${userID}/${productID}`,
      {
        method: "DELETE",
      }
    );
